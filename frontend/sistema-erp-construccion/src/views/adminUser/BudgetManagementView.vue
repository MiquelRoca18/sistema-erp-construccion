<template>
    <div class="flex flex-col justify-center items-center min-h-screen p-8">
      <div class="max-w-5xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg transition-colors">
        <!-- Encabezado y acción -->
        <div class="flex flex-col sm:flex-row items-center justify-between mb-6">
          <h1 class="text-3xl font-bold text-yellow-800 dark:text-yellow-200 text-center sm:text-left mb-4 sm:mb-0">
            Gestión de Presupuestos
          </h1>
          <button
            @click="openCreateModal"
            class="px-4 py-2 bg-yellow-600 dark:bg-yellow-500 text-white rounded-lg hover:bg-yellow-700 dark:hover:bg-yellow-600 transition"
          >
            Nuevo Presupuesto
          </button>
        </div>
        ...
        <div class="mb-6 w-full grid grid-cols-1 md:grid-cols-2 gap-4">
          <input
            ...
            class="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 transition"
          />
          <input
            type="number"
        ...
        <div class="block sm:hidden w-full">
          <div
            ...
            class="bg-white dark:bg-gray-700 p-4 rounded-lg shadow mb-4 cursor-pointer transition-colors hover:bg-yellow-100 dark:hover:bg-yellow-800"
          ...
          >
            <div class="flex justify-between items-center">
              ...
              <div class="flex space-x-2">
                ...
                <button
                  @click.stop="openDeleteModal(budget)"
          ...
          <div v-if="paginatedBudgets.length === 0 && !loading" class="text-center text-gray-500">
        ...
        <div class="hidden sm:block w-full overflow-x-auto">
          <table class="min-w-full">
            <thead>
              <tr class="bg-gradient-to-r from-yellow-100 to-yellow-200 text-yellow-800 dark:text-yellow-200">
            ...
            <tbody>
              <tr
                v-for="budget in paginatedBudgets"
                :key="budget.presupuestos_id"
                class="bg-white dark:bg-gray-700 shadow rounded-lg transition-colors cursor-pointer hover:bg-yellow-100 dark:hover:bg-yellow-800"
                @click="openViewModal(budget)"
              >
                <td class="px-6 py-4">{{ budget.presupuestos_id }}</td>
                <td class="px-6 py-4">{{ budget.nombre_proyecto }}</td>
                <td class="px-6 py-4 hidden [@media(min-width:1200px)]:table-cell">{{ budget.equipos }}</td>
                <td class="px-6 py-4 hidden [@media(min-width:1200px)]:table-cell">{{ budget.mano_obra }}</td>
                <td class="px-6 py-4 hidden [@media(min-width:1200px)]:table-cell">{{ budget.materiales }}</td>
                <td class="px-6 py-4 font-bold">{{ budget.total }}</td>
                <td class="px-6 py-4">
        ...
        <div v-if="totalPages > 1" class="mt-6 flex items-center justify-center space-x-2">
          <button 
            ...
            class="flex items-center justify-center w-10 h-10 rounded-full bg-yellow-600 dark:bg-yellow-500 text-white hover:bg-yellow-700 dark:hover:bg-yellow-600 disabled:opacity-50 transition"
          >
          ...
          <div class="flex space-x-2">
            <button 
              ...
              :key="page" 
              @click="goToPage(page)" 
              class="w-10 h-10 rounded-full border border-yellow-600 text-yellow-600 flex items-center justify-center transition"
              :class="page === currentPage ? 'bg-yellow-600 text-white' : 'bg-white hover:bg-yellow-600 hover:text-white'"
            >
              {{ page }}
            </button>
          </div>
      ...
      <!-- Modales -->
      <CreateBudgetModal v-if="showCreateModal" @close="closeCreateModal" @created="fetchBudgets" />
      <EditBudgetModal v-if="showEditModal" :budget="budgetToEdit" @close="closeEditModal" @updated="fetchBudgets" />
      <DeleteBudgetModal v-if="showDeleteModal" :budget="budgetToDelete" @close="closeDeleteModal" @deleted="fetchBudgets" />
    </div>
  </template>
    
  <script setup lang="ts">
  import { ref, computed, onMounted, watch } from 'vue';
  import { getBudgets, deleteBudget } from '@/service/budgetService';
  import CreateBudgetModal from '@/components/adminUser/CreateBudgetModal.vue';
  import EditBudgetModal from '@/components/adminUser/EditBudgetModal.vue';
  import DeleteBudgetModal from '@/components/adminUser/DeleteBudgetModal.vue';
  ...
  const deleteBudgetConfirmed = async () => {
    ...
    try {
      await deleteBudget(budgetToDelete.value.presupuestos_id);
      fetchBudgets();
    } catch (err: any) {
