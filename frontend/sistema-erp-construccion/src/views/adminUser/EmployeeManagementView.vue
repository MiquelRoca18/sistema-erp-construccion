<template>
  <div class="flex flex-col justify-center items-center min-h-screen p-4 md:p-8 transition-colors duration-300">
    <div class="w-full max-w-5xl mx-auto bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow-lg dark:shadow-gray-900/30 transition-colors duration-300">
      ...
      <div v-else-if="!loading && (!employees || employees.length === 0)" class="p-8 text-center">
        <p class="text-gray-500 dark:text-gray-400">No hay empleados disponibles. Comienza añadiendo uno nuevo.</p>
      </div>
...
const totalPages = computed(() => {
  if (!filteredEmployees.value || filteredEmployees.value.length === 0) return 1;
  return Math.ceil(filteredEmployees.value.length / pageSize);
});

const paginatedEmployees = computed(() => {
  if (!filteredEmployees.value || filteredEmployees.value.length === 0) return [];
