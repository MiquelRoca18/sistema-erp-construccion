<template>
  <div class="w-full h-full flex flex-col justify-center px-3 sm:px-4 md:px-6 min-h-screen transition-colors duration-300">
    <!-- Panel de Filtros -->
    <div class="mx-auto w-full max-w-5xl mb-3 sm:mb-4">
      <details class="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg shadow-lg dark:shadow-gray-900/30 transition-colors duration-300">
        <summary class="cursor-pointer px-3 sm:px-4 py-2 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-t-md select-none flex items-center justify-between transition-colors duration-300">
          <span class="text-base sm:text-lg font-semibold">Filtrar Tareas</span>
        ...
        <div class="px-3 py-3 sm:px-4 sm:py-4 md:px-6 md:py-6 dark:bg-gray-800 transition-colors duration-300">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <!-- Filtro de Proyecto -->
            <div>
              <label class="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Proyecto</label>
              <select 
                v-model="selectedProject" 
                @change="fetchTasks" 
                class="w-full p-2 sm:p-2.5 border border-gray-300 dark:border-gray-600 rounded-md text-xs sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-colors duration-300"
                :disabled="loading"
              >
            ...
            <!-- Filtro de Estado -->
            <div>
              <label class="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Estado</label>
              <select 
            ...
            <div>
              <label class="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tipo de Tarea</label>
              <select 
            ...
            <div>
              <label class="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fecha de Inicio</label>
              <input 
                v-model="startDate" 
                @change="fetchTasks" 
                type="date" 
    ...
    <!-- Lista de Tareas -->
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg dark:shadow-gray-900/30 p-3 sm:p-4 md:p-6 mx-auto w-full max-w-5xl flex flex-col min-h-[500px] sm:min-h-[600px] transition-colors duration-300">
      <h2 class="text-xl sm:text-2xl font-semibold text-gray-800 dark:text-white mb-3 sm:mb-4 text-center">Lista de Tareas</h2>

      <div v-if="loading" class="text-center flex-grow flex flex-col items-center justify-center">
        <!-- Loader animado -->
        <div class="w-12 sm:w-16 h-12 sm:h-16 mb-3 sm:mb-4">
        ...
        <p class="text-base sm:text-lg text-gray-500 dark:text-gray-400">Cargando tareas...</p>
      </div>
      
      <div v-if="error" class="text-red-500 dark:text-red-400 text-center text-base sm:text-lg flex-grow flex items-center justify-center">
        {{ error }}
      </div>
      ...
      <!-- Grid de Tareas -->
      <div v-if="!loading && filteredTasks.length > 0" class="grid gap-3 sm:gap-5 flex-grow" :class="(isMobile || isTablet) ? 'grid-cols-1 grid-rows-3' : 'grid-cols-2 grid-rows-3'">
        <div
        ...
        >
          ...
          <div class="px-3 sm:px-6 py-3 sm:py-4 border-b border-gray-100 dark:border-gray-600">
            <div class="flex justify-between items-center">
              <h3 class="font-bold text-base sm:text-lg text-gray-800 dark:text-white truncate pr-2">
                {{ task.nombre_tarea }}
              </h3>
              <span
                class="inline-block px-2 sm:px-3 py-1 text-xs font-semibold rounded-full flex-shrink-0"
                :class="{
      ...
      <!-- Paginación -->
      <div v-if="!loading && totalPages > 1" class="mt-4 sm:mt-6 flex items-center justify-center space-x-1 sm:space-x-2">
        <!-- Botón Anterior -->
        <button 
          @click="prevPage" 
          :disabled="currentPage === 1"
          class="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-blue-600 dark:bg-blue-500 text-white hover:bg-blue-700 dark:hover:bg-blue-600 disabled:opacity-50 transition-colors duration-300"
        >
        ...
        <div class="flex space-x-1 sm:space-x-2">
          <button 
            ...
            :key="page" 
            @click="goToPage(page)" 
            class="w-7 h-7 sm:w-10 sm:h-10 rounded-full border border-blue-600 dark:border-blue-500 flex items-center justify-center transition-colors duration-300 font-medium text-xs sm:text-base"
...
/* Ajuste para inputs de fecha en móviles */
