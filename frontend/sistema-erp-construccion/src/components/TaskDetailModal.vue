...
<template>
  ...
  // Sincronizar el estado en caso de que se cambie la tarea al reabrir el modal.
  ...
  // Función para actualizar la tarea mediante la llamada PUT al backend.
  const updateTask = async () => {
    try {
      const data = { estado: updatedStatus.value };
      await updateTaskService(props.task.tareas_id, data);
      // Emitir el evento 'update' con la tarea actualizada para refrescar la vista principal.
