    ...
    <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden transform transition-all duration-300">
      ...
      <div class="bg-gradient-to-r from-orange-500 to-orange-400 p-4">
        <div class="flex justify-between items-center">
          ...
          <button @click="close" class="text-white text-3xl leading-none hover:text-gray-200 cursor-pointer">&times;</button>
        </div>
      </div>
      <!-- Contenido -->
      <div class="p-6 space-y-6">
        <!-- Nombre y Estado -->
        <div>
          <h3 class="text-gray-800 text-xl font-semibold mb-1">{{ task.nombre_tarea }}</h3>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div v-if="task.fecha_inicio">
            ...
            <p class="text-gray-800 font-medium">{{ formatDate(task.fecha_inicio) }}</p>
          </div>
          <div v-if="task.fecha_fin">
            <p class="text-gray-600 text-sm">
              <span class="font-medium">Fin:</span>
            </p>
            <p class="text-gray-800 font-medium">{{ formatDate(task.fecha_fin) }}</p>
          </div>
        </div>
        <!-- Descripción -->
        <div v-if="task.descripcion">
          <p class="text-gray-600 text-sm font-medium mb-1">Descripción:</p>
          <p class="text-gray-800 leading-relaxed">{{ task.descripcion }}</p>
        </div>
        <!-- Responsables -->
        <div>
          <p class="text-gray-600 text-sm">
          ...
          <p class="text-gray-800 font-medium">{{ task.empleado_nombre }}</p>
        </div>
      </div>
      <!-- Pie de modal -->
      <div class="bg-gray-100 p-4 text-right">
        <button @click="close" class="px-4 py-2 bg-orange-500 text-white rounded hover:bg-orange-600 transition">
          Cerrar
        </button>
      </div>
    </div>
