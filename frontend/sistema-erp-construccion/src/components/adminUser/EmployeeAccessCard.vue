<template>
  <router-link to="/employees" class="block">
    <div class="p-6 bg-gradient-to-r from-sky-100 to-sky-200 rounded-lg shadow-lg transform transition hover:scale-105">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-2xl font-bold text-sky-800">Empleados</h2>
        </div>
        <div>
          <!-- Ícono representativo de empleados -->
          <svg class="w-12 h-12 text-sky-800" fill="currentColor" viewBox="0 0 24 24">
            <path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5s-3 1.34-3 3 1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 2.04 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z"/>
          </svg>
        </div>
      </div>
    </div>
  </router-link>
</template>

<script setup lang="ts">
import { ref, onMounted } from 'vue';
import { getEmployees } from '@/service/employeeService';
// Se sigue consultando el total aunque no se muestre en el card
const totalEmployees = ref(0);
const fetchTotalEmployees = async () => {
  try {
    const employees = await getEmployees();
    totalEmployees.value = employees.length;
  } catch (err: any) {
    console.error("Error al obtener empleados:", err);
  }
};
onMounted(() => {
  fetchTotalEmployees();
});
</script>

<style scoped>
/* Ajustes opcionales */
</style>
