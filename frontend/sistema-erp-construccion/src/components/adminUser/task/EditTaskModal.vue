<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 dark:bg-black/70">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-[95%] max-w-md overflow-hidden transform transition-all duration-300 max-h-[80vh] overflow-y-auto">
      ...
      <!-- Formulario -->
      <form v-else @submit.prevent="handleSubmit" class="space-y-4 p-4">
        <!-- Nombre de la Tarea -->
        <div>
        ...
        <div>
          ...
          <div v-if="projectsLoading" class="flex items-center space-x-2 px-3 py-2 border rounded-md border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700">
            ...
            <span class="text-gray-600 dark:text-gray-300 text-sm">Cargando proyectos...</span>
          </div>
        ...
        <!-- Mensaje de error -->
        <div v-if="errorMessage" class="p-2 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-md text-red-600 dark:text-red-300 text-sm flex items-center">
        ...
        <div class="flex justify-end space-x-2">
          ...
          <button
            type="submit"
            class="px-3 py-2 bg-orange-600 dark:bg-orange-500 text-white rounded hover:bg-orange-700 dark:hover:bg-orange-600 transition text-xs flex items-center justify-center min-w-[80px]"
