      ...
      <div class="p-6 space-y-6">
        ...
        <div v-if="loading" class="flex flex-col items-center justify-center py-4">
          <div class="relative mb-3">
            <div class="h-12 w-12 rounded-full border-t-4 border-b-4 border-red-500 animate-spin"></div>
            <div class="absolute top-0 left-0 h-12 w-12 rounded-full border-t-4 border-b-4 border-red-200 animate-spin" style="animation-duration: 1.5s;"></div>
          </div>
          <p class="text-gray-600 dark:text-gray-300">Eliminando tarea...</p>
        </div>
        
        <!-- Mensaje de confirmación -->
        <p v-else class="text-gray-700 dark:text-gray-200 flex items-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
          </svg>
          ¿Estás seguro de que deseas eliminar la tarea
          <span class="font-bold mx-1">{{ task.nombre_tarea }}</span>?
        </p>
        ...
        <div class="flex justify-end space-x-4">
          ...
          <button
            ...
            class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition text-sm dark:bg-red-500 dark:hover:bg-red-600 min-w-[90px] flex items-center justify-center"
            :disabled="loading"
          >
            <span v-if="loading">
