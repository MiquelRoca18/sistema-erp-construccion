    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all duration-300">
      ...
      <div class="bg-gradient-to-r from-green-500 to-green-400 dark:from-green-700 dark:to-green-600 p-3 sm:p-4 rounded-t-2xl">
        <div class="flex justify-between items-center">
          <h2 class="text-white text-xl sm:text-2xl font-bold">Editar Proyecto</h2>
          <button 
            @click="closeModal" 
      ...
      <div v-if="employeesLoading" class="p-4 sm:p-6 flex flex-col items-center justify-center">
        ...
        <p class="text-sm sm:text-base text-gray-600 dark:text-gray-300">Cargando datos...</p>
      </div>
      
      <!-- Formulario -->
      <form v-else @submit.prevent="handleSubmit" class="p-4 sm:p-6 space-y-4 sm:space-y-6">
        <!-- Responsable -->
        <div class="mb-3 sm:mb-4">
          <label class="block text-sm sm:text-base text-gray-700 dark:text-gray-200">Responsable</label>
          <select 
            v-model="form.responsable_id" 
            required 
            :disabled="loading"
            class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-400 text-sm sm:text-base
        ...
        <!-- Nombre del Proyecto -->
        <div class="mb-3 sm:mb-4">
          <label class="block text-sm sm:text-base text-gray-700 dark:text-gray-200">Nombre del Proyecto</label>
          <input 
        ...
        <!-- Estado -->
        <div class="mb-3 sm:mb-4">
          <label class="block text-sm sm:text-base text-gray-700 dark:text-gray-200">Estado</label>
          <select 
            v-model="form.estado" 
            required 
            :disabled="loading"
        ...
        <!-- Fechas en grid para móvil y desktop -->
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          <!-- Fecha de Inicio -->
          <div>
            <label class="block text-sm sm:text-base text-gray-700 dark:text-gray-200">Fecha de Inicio</label>
            <input 
          ...
          <div>
            <label class="block text-sm sm:text-base text-gray-700 dark:text-gray-200">Fecha de Fin</label>
            <input 
        ...
        <!-- Descripción -->
        <div class="mb-3 sm:mb-4">
          <label class="block text-sm sm:text-base text-gray-700 dark:text-gray-200">Descripción</label>
          <textarea 
        ...
        <div class="flex justify-end space-x-3 sm:space-x-4">
          ...
          <button 
            type="submit" 
            :disabled="loading"
            class="px-3 sm:px-4 py-1.5 sm:py-2 bg-green-600 dark:bg-green-500 text-white rounded hover:bg-green-700 dark:hover:bg-green-600 transition text-xs sm:text-sm
                  disabled:opacity-70 disabled:cursor-not-allowed
                  flex items-center justify-center min-w-[80px] sm:min-w-[120px]"
          >
