<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 dark:bg-black/70 p-4">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all duration-300">
      <!-- Encabezado -->
      <div class="bg-gradient-to-r from-red-500 to-red-400 dark:from-red-700 dark:to-red-600 p-3 sm:p-4 rounded-t-2xl">
        <div class="flex justify-between items-center">
          <h2 class="text-white text-lg sm:text-2xl font-bold">Confirmar Borrado</h2>
      ...
      <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
        ...
        <div class="flex justify-end space-x-2 sm:space-x-4">
          ...
          <button
            @click="confirmDelete"
            :disabled="loading"
            class="px-2 py-1 sm:px-4 sm:py-2 bg-red-600 text-white rounded hover:bg-red-700 transition text-xs sm:text-sm dark:bg-red-500 dark:hover:bg-red-600 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center"
          >
